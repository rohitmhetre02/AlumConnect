...
const UserTopbar = ({ onToggleSidebar }) => {
  ...
  const lastViewModeRef = useRef('list')
  ...
  const ensureConversationLoaded = useCallback(
    ...
    [conversationMap, getConversationMessages]
  ...
  const showConversation = useCallback(
    ...
    (conversationId) => {
      ...
      lastViewModeRef.current = 'conversation'
  ...
  const openMessagesPanel = useCallback(
    ...
    (conversationId) => {
      ...
      } else if (lastViewModeRef.current === 'conversation' && lastConversationIdRef.current) {

        showConversation(lastConversationIdRef.current)
  ...
  const closeMessagesPanel = useCallback(() => {
    ...
    lastViewModeRef.current = messagesViewMode

    setIsMessagesOpen(false)

  }, [activeConversationId, messagesViewMode])
  ...
  useEffect(() => {
    ...
    const handleOpenMessages = (event) => {
      ...
      openMessagesPanel(conversationId)

    }

    window.addEventListener('app:openMessagesPanel', handleOpenMessages)

    return () => window.removeEventListener('app:openMessagesPanel', handleOpenMessages)
  ...
  }, [isMessagesOpen, activeConversationId, messagesViewMode])
...
const MessagesPanel = ({
  ...
  // Get the active conversation from contacts

  const activeContact = contacts.find((item) => item._id === activeConversationId)
  ...
  const listCards = contacts.map((contact) => {
    ...
    return (
      ...
      <button
        ...
        onClick={() => onSelectConversation(contact._id)}
  ...
  return (
    ...
    <SlideOverBase
      ...
      title={showChat && activeContact ? getOtherParticipant(activeContact)?.name || 'Chat' : 'Your conversations'}
          ...
          {activeContact && (
            ...
            <div className="flex items-center gap-3 rounded-2xl bg-slate-100/80 px-4 py-3">
              ...
              <div className="h-11 w-11 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold">
                ...
                {getOtherParticipant(activeContact)?.name?.charAt(0) || '?'}
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-900">
                  ...
                  {getOtherParticipant(activeContact)?.name || 'Unknown'}

                </p>

                <p className="text-xs text-slate-500 capitalize">

                  {getOtherParticipant(activeContact)?.role || 'User'}
          ...
          <div className="space-y-3">
            ...
            {conversation.length ? (
              ...
              conversation.map((entry) => {
                ...
                const isMe = entry.senderId === user?.id
