...
const ContentPosted = () => {
  ...
  const handleEdit = (content) => {
    const editRoutes = {
      opportunity: `/dashboard/opportunities/${content.id}/edit`,
      event: `/dashboard/events/${content.id}/edit`,
      donation: `/dashboard/donations/${content.id}/edit`,
      campaign: `/dashboard/campaigns/${content.id}/edit`
    }
  ...
  const handleView = (content) => {
    const viewRoutes = {
      opportunity: `/dashboard/opportunities/${content.id}`,
      event: `/dashboard/events/${content.id}`,
      donation: `/dashboard/donations/${content.id}`,
      campaign: `/dashboard/campaigns/${content.id}`
    }
    navigate(viewRoutes[content.contentType])
  }

  const handleApplications = (content) => {
    // Navigate to applications page for opportunities
    navigate(`/dashboard/opportunities/${content.id}/applications`)
  }

  const handleRegistrations = (content) => {
    // Navigate to registrations page for events
    navigate(`/dashboard/events/${content.id}/registrations`)
  }

  const handleDonations = (content) => {
    // Navigate to donations page for campaigns
    navigate(`/dashboard/campaigns/${content.id}/donations`)
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-purple-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-purple-700 hover:shadow-md"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
