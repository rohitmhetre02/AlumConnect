...
import { CheckCircle2, ClipboardList, Clock, RefreshCw, ShieldAlert, Users2 } from 'lucide-react'
...
const emptyStateCopy = {
  faculty: {
    title: 'No faculty awaiting approval',
    message: 'All faculty registrations have been reviewed and actioned.',
  },
  coordinator: {
    title: 'No coordinator requests pending',
    message: 'No department coordinator registrations require action right now.',
  },
  all: {
    title: 'No pending registrations',
    message: 'New faculty and coordinator registrations will surface here for review.',
  },
}
...
const StatsCard = ({ icon: Icon, title, count, accent }) => (
  <div className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-soft ${accent || ''}`}>
    <div className="flex items-center gap-3">
      <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900/5 text-slate-700">
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap gap-2 pt-2 text-xs">
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
              {entry.role === 'faculty' ? 'Faculty' : 'Coordinator'}
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">
      ...
      <td className="px-4 py-4 align-top">
        <div className="grid gap-1 text-xs text-slate-600">
          {entry.metadata.title ? (
            <span><span className="font-medium">Title:</span> {entry.metadata.title}</span>
          ) : null}
          {entry.metadata.phone ? (
            <span><span className="font-medium">Phone:</span> {entry.metadata.phone}</span>
          ) : null}
...
const AdminRegistrationApprovals = () => {
  const { user } = useAuth()
  ...
  const [pendingData, setPendingData] = useState({ faculty: [], coordinators: [] })
  const [stats, setStats] = useState({ faculty: { pending: 0 }, coordinator: { pending: 0 } })
  const [decisionLoading, setDecisionLoading] = useState(false)

  const loadData = useCallback(async () => {
    try {
      const [pendingResponse, statsResponse] = await Promise.all([
        getAdminPending(roleFilter),
        getAdminStats(),
      ])
  ...
  }, [addToast, getAdminPending, getAdminStats, roleFilter])
  ...
  const combinedEntries = useMemo(() => {
    if (roleFilter === 'faculty') return pendingData.faculty ?? []
    if (roleFilter === 'coordinator') return pendingData.coordinators ?? []
    return [...(pendingData.faculty ?? []), ...(pendingData.coordinators ?? [])]
  }, [pendingData.faculty, pendingData.coordinators, roleFilter])
  ...
  const handleDecision = async (userId, role, action) => {
    try {
      ...
      await adminDecision({ userId, role, action, reason })
  ...
  return (
    <section className="space-y-8">
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-bold text-slate-900">Platform Registration Approvals</h1>
        <p className="max-w-2xl text-sm text-slate-600">
          Review and action new faculty and department coordinator registrations across AlumConnect. Approved coordinators will gain access to department dashboards immediately.
        </p>
      </header>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard icon={Users2} title="Pending Faculty" count={stats.faculty?.pending ?? 0} />
        <StatsCard icon={ClipboardList} title="Pending Coordinators" count={stats.coordinator?.pending ?? 0} />
        <StatsCard icon={Clock} title="Total Pending" count={combinedEntries.length} />
        <StatsCard icon={ShieldAlert} title="Decisions Today" count={0} accent="bg-slate-900/5" />
      </div>
...
export default AdminRegistrationApprovals
