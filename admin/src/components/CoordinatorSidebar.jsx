...
const CoordinatorSidebar = ({ isMobile = false, onClose }) => {
  ...
  const [userName, setUserName] = useState('Coordinator User')
  ...
  // Coordinator always uses /coordinator path
  ...
  const normalizedRole = userRole === 'coordinator' ? 'Coordinator' : 'User'
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserName(user.name || 'Coordinator User')
      setUserAvatar(user.name ? user.name.charAt(0).toUpperCase() : 'C')
      setProfileApprovalStatus(user.profileApprovalStatus || 'APPROVED')
      setAvatarUrl(user.profilePicture || '')
      console.log('Coordinator sidebar user data loaded:', { name: user.name, role: user.role, profileApprovalStatus: user.profileApprovalStatus })
    } else {
  ...
  const NavItem = ({ icon, label, path, subOptions, id }) => {
    ...
    return (
      <div className="mb-1">
        <div 
          onClick={() => hasSub ? toggleMenu(label) : null}
        ...
        >
          <NavLink to={hasSub ? '#' : path} className="flex items-center flex-1">
            <span className={`mr-3 ${isActive(path) && !hasSub ? 'text-blue-600' : 'text-slate-400 group-hover:text-slate-600'}`}>
              {icon}
            </span>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 bg-white shadow-xl lg:relative lg:shadow-none lg:z-auto transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 tracking-widest uppercase font-semibold">Coordinator Portal</p>
              </div>
            </div>
            {isMobile && (
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-10 scrollbar-hide">
            <div className="mb-6">
              <p className="px-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Your Dashboard</p>
            ...
            <div className="mb-6">
              ...
              <NavItem 
                ...
                label="News" 
                path={`${basePath}/news`} />
              <NavItem icon={<Image size={18} />} label="Gallery" path={`${basePath}/gallery`} />
            </div>
