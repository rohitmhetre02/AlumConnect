...
const OpportunityApplicants = () => {
  ...
  const { data: applicants, isLoading: dataLoading, error, refetch } = useApi(`/opportunities/${opportunityId}/applicants`)
  ...
  const handleUpdateStatus = async (applicantId, newStatus, note = '') => {
    setIsLoading(true)
    try {
      await api.put(`/opportunities/referrals/${applicantId}/status`, { 
        status: newStatus, 
        reviewerNote: note 
      })
      await refetch()
    ...
    <div className="space-y-6">
      <header className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Opportunity Applicants</h1>
            <p className="text-slate-600 mt-1">View and manage applications for this opportunity.</p>
          </div>
          <button 
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-700">Referral Status</th>
              ...
              {filteredApplicants.length === 0 ? (
                ...
                filteredApplicants.map((applicant) => (
                  <tr key={applicant.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${getStatusBadgeClass(applicant.referralStatus || applicant.status)}`}>
                        {(applicant.referralStatus || applicant.status).charAt(0).toUpperCase() + (applicant.referralStatus || applicant.status).slice(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {applicant.resumeUrl ? (
